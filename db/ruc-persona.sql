...
begin
  ...
  -- ── NO SE PISA UN NÚMERO YA CARGADO ──
  -- Si la ficha ya tiene otro, esto dejó de ser «completar un hueco» y pasó a
  -- ser «corregir un dato», que se hace en la ficha, mirándola entera. Aquí
  -- estaríamos cambiando la identidad tributaria de alguien desde un pop-up de
  -- adjuntar archivos.
  ...
  select nombre into v_otra from personas
   where id <> p_persona and regexp_replace(coalesce(ruc_dni, ''), '\D', '', 'g') = v_ruc
   limit 1;

-- ============================================================
--  db/compromiso-acta-po001-cargar.sql — LAS 39 FILAS DEL ACTA 139-2025-DAFO
--  PO-001 · Mujeres del Ande: Voces que Transforman
--
--  UNA SOLA SENTENCIA. Córrelo de arriba abajo; no hay nada que descomentar.
--
--  ── POR QUÉ UNA SOLA SENTENCIA Y NO UNA TABLA DE PASO ──
--  La primera versión de esta carga hacía `create table comp_139_2025` y luego
--  insertaba desde ella, copiando el patrón de db/compromiso-acta.sql. Falló
--  dos veces seguidas por la misma razón de fondo: un archivo con varias
--  sentencias dependientes se puede correr a medias —una selección en el
--  editor, un paso que se salta— y entonces o no escribe nada (y la pantalla
--  queda vacía sin dar error) o revienta con «relation does not exist».
--  Así no: el `values` viaja dentro del propio insert, el id se resuelve por
--  código en la misma sentencia, y no hay estado intermedio que se pueda
--  perder. Si se corre, se corre entero; si no, no escribe nada. Es la misma
--  regla que ya está escrita en CARGAR-CRONOGRAMA.md.
--
--  ── DE DÓNDE SALEN LOS DATOS ──
--  Del PDF del acta firmada, 12 páginas sin capa de texto (`pdftotext` devuelve
--  12 bytes: escaneo puro), pasado a 300 ppp y leído con OCR en español. Los
--  `detalle` están pegados a la letra del acta, no resumidos. La lectura
--  completa y el porqué de cada decisión están en db/compromiso-acta-po001.sql.
--
--  19 entregables · 14 obligaciones · 6 plazos.
--
--  Idempotente: `on conflict do nothing`. Correrlo dos veces no duplica nada y
--  no pisa lo que alguien ya haya marcado como entregado.
--
...
-- ------------------------------------------------------------
--  VERIFICAR — tiene que devolver TRES filas: 19 · 14 · 6
--
--  Si devuelve CERO filas, el insert no escribió nada. La causa casi segura es
--  que 'PO-001' no exista con ese código exacto: el `from (select …) po` no
--  falla cuando no encuentra la postulación, simplemente no produce ninguna
--  fila que insertar. Compruébalo con:
--     select codigo, estado from postulaciones where codigo = 'PO-001';
-- ------------------------------------------------------------

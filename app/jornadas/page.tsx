...
export default async function Jornadas({ searchParams }: { searchParams: { m?: string } }) {
  ...
  return (
    <div className="shell" style={{ maxWidth: "min(1040px, 96vw)" }}>
      ...
      {/* ── EL REPARTO POR SEMANAS, DESPUÉS DEL DÍA A DÍA ──
          Estaba antes del detalle diario, entre las tres tarjetas del pago y la
          lista de días. Ahí partía en dos la misma pregunta: arriba «cuánto
          cobro», en medio «cómo se reparte», y abajo «qué días trabajé» — que
          es de donde salen las dos cifras de arriba.
          El orden ahora va de lo general a lo detallado y no vuelve atrás:
          el total del mes, los días uno a uno, y al final el reparto por
          semanas, que es el resumen de lo que se acaba de leer. */}
      <div className="h4" style={{ marginTop: 18 }}>📅 Detalle del pago por semana</div>

...
/* ══════════════════════════════════════════════════════════════════════════
   LA TARJETA DE UN CASO — el caso entero, sin abrirlo

   Era `PostCard`, la tarjeta del feed de la portada. Cuando la portada dejó de
   listar casos se quedó sin nadie que la montara y se borró; vuelve porque la
   ficha de una ETIQUETA la necesita: entrar a «Rodaje» es entrar a LEER de qué
   van sus casos, y una fila de una línea obliga a abrir cinco pestañas para
   enterarse de lo mismo.

   Frente a la fila corta (`cardPub` en la ficha de entidad) esta enseña el
   cuerpo, las imágenes, los vínculos, las reacciones y deja cambiar tipo y
   estado sin salir. Frente a la ficha del caso, no tiene el hilo: para
   conversar se entra, o se abre la vista rápida.

   ── LO QUE NO VOLVIÓ CON ELLA ──
   · el ✨ NUEVO (marcaba lo publicado desde tu última visita AL FEED, y ese
     feed ya no existe);
   · el ojo de «ocultar de mi feed», por lo mismo — aquí no hay un feed
     personal del que esconder nada;
   · las marcas 📤/👁 de «lo delegué / me mencionan», que eran de la vista
     «Mis asuntos» y hoy vive en el tablero.
   Volver a traerlas sería arrastrar el vocabulario de una pantalla que ya no
   está.

   (TIPOS_SEL salió a lib/tipos, con el mismo orden y la misma decisión: no se
   ofrece «conversación» — es el cajón donde cae lo que nadie clasificó, no
   algo que uno elija.)
   ══════════════════════════════════════════════════════════════════════════ */
export default function TarjetaCaso({
  ...
  hijos, equipoTotal, prog, autorId,
}: {
  ...
  /** Quién lo escribió. Para el acuse de un aviso: el autor cuenta como
   *  enterado —lo escribió él— y no se le vuelve a preguntar. Sin esto, el
   *  mismo aviso decía «3/8» en el muro y «2/8» aquí. */
  autorId?: string | null;
  equipoTotal?: number;
  ...
  /* Los que pusieron 👀, MÁS el autor si es un aviso: escribirlo es la forma
     más fuerte de estar enterado, y pedírselo era pedirle que se entere de lo
     que acaba de decir. Se cuenta una sola vez aunque además reaccionara. */
  const vistos = new Set((reacciones || []).filter(r => r.emoji === "👀").map(r => r.usuario_id));
  if (llevaEnterado(tipo) && autorId) vistos.add(autorId);
  const enterN = vistos.size;
  const esAutorAviso = !!autorId && autorId === userId;
  const enterMio = (reacciones || []).some(r => r.emoji === "👀" && r.usuario_id === userId);
  return (
    /* ── UN ENLACE, NO UN `onClick` ──
       La tarjeta navegaba con `router.push`, y eso es un enlace que solo
       funciona con el clic izquierdo: ni ctrl+clic, ni rueda, ni «abrir en
       pestaña nueva», ni destino en la barra de estado, ni teclado. En la
       ficha de una etiqueta —donde se repasan varios casos— abrir tres en
       paralelo es EL gesto. Enlace estirado (`fila-cubre`), como el resto del
       sistema; lo interactivo de dentro se levanta con `fila-encima`. */
    <div className={`card link est-${claseEstado(estado, tipo)} ${estado === "resuelta" ? "card-apagada" : ""}`}
      style={{ position: "relative" }}>
      <Link href={href} className="fila-cubre" aria-label={titulo} />
      <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {(imagenes || []).length > 0 && (
            <div className="fila-encima" style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 9 }}>
          ...
          {chips.length > 0 && (
            <div className="sel-chips fila-encima" style={{ marginTop: 9 }}>
              {chips.map((c, i) => (
          ...
          {/* `fila-encima`: el pie es todo botones —reaccionar, cambiar tipo,
              cambiar estado, vista rápida— y el enlace estirado de la tarjeta
              está por encima de todo lo que no se levante. */}
          <div className="post-pie fila-encima" style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {llevaEnterado(tipo) && pubId && userId && (
              <AvisoMini pubId={pubId} enterados={enterN} total={equipoTotal}
                mio={enterMio} esAutor={esAutorAviso} />
            )}
            {pubId && userId && (

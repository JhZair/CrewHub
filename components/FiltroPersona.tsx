...
/* Filtro compacto por persona para el Tablero: vive en la esquina
   ...
   `sufijo` preserva el modo de vista (ej. modo=timeline) al navegar. */
export default function FiltroPersona({ equipo, actual, sufijo = "" }: {
  equipo: { id: string; nombre: string }[]; actual: string; sufijo?: string;
}) {
  const router = useRouter();
  return (
    <select className="sel-persona" value={actual}
      title="Ver los asuntos de una persona del equipo"
      onChange={e => router.push(`/tablero?p=${e.target.value}${sufijo ? "&" + sufijo : ""}`)}>

...
/* ══════════════════════════════════════════════════════════════════════════
   LA PREGUNTA AL REVÉS: ¿QUÉ HAY EN EL CAJÓN 07?

   Cuenta lo que cuelga de cada sitio, DIRECTO y TOTAL. Los dos números hacen
   falta y dicen cosas distintas: en el Cajón 08 hay «2 cosas» —dos bolsos— y
   dentro de ellos hay treinta equipos. Abrir el cajón enseña dos; buscar el
   trípode que está ahí dentro necesita el treinta.
   ══════════════════════════════════════════════════════════════════════════ */
export function contarPorSitio(
  sitios: Sitio[], equipos: EqGuardable[],
): Map<string, { directo: number; total: number }> {
  const r = resolvedorDeGuardado(sitios, equipos);
  const m = new Map<string, { directo: number; total: number }>();
  (sitios || []).forEach(s => m.set(s.id, { directo: 0, total: 0 }));

  for (const e of equipos || []) {
    const g = r.de(e.id);
    if (!g.ruta.length) continue;
    /* El sitio más CERCANO de la ruta —el último `sitio` antes de los equipos—
       es el que lo contiene de verdad; los de más afuera lo contienen a través
       de él, y por eso suman en `total` pero no en `directo`. */
    const sitiosEnRuta = g.ruta.filter(t => t.tipo === "sitio");
    if (!sitiosEnRuta.length) continue;
    const cercano = sitiosEnRuta[sitiosEnRuta.length - 1];
    /* `directo` solo si el equipo cuelga del sitio SIN bolsos de por medio:
       con un contenedor en la ruta, quien está directo en el cajón es el bolso,
       no lo que lleva dentro. */
    const hayContenedor = g.ruta.some(t => t.tipo === "equipo");
    const c = m.get(cercano.id);
    if (c && !hayContenedor) c.directo++;
    for (const t of sitiosEnRuta) {
      const x = m.get(t.id);
      if (x) x.total++;
    }
  }
